...
export function listVisibleOptions(options, optionState) {
  return options.filter(option => {
    ...
    const value = get(optionState, parentOption.id) || get(optionState, parentOption.name)
...
function matchByToggleValue(option, stateValue) {
  return stateValue === true || stateValue === option.name || stateValue === option.id
}
...
function matchBySelectValueId(option, stateValue) {
  return Array.isArray(option.parentValueIds) && option.parentValueIds.includes(stateValue)
}
...
function matchBySelectValueName(option, parentOption, stateValue) {
  return (
    ...
    option.parentValueIds
      .map(id => {
        const parentOptionValue = parentOption.values.find(value => value.id === id)
